...
exports.signup = (request, response) => {
  ...
  let token, userId;
  db.doc(`/${constants.collection.users}/${user.handle}`)
    .get()
    .then((doc) => {
      if (doc.exists) {
        return response
    ...
    .then((idToken) => {
      ...
      return db.doc(`/${constants.collection.users}/${user.handle}`).set(userCredentials);
    })
    .then(() => {
...
exports.addUserDetails = (request, response) => {
  ...
  db.doc(`/${constants.collection.users}/${request.user.handle}`)
    .update(userDetails)
    .then(() => {
...
exports.getUserDetails = (request, response) => {
  let userData = {};
  db.doc(`/${constants.collection.users}/${request.params.handle}`)
    .get()
    .then((doc) => {
      if (doc.exists) {
        userData.user = doc.data();
        return db
          .collection(constants.collection.screams)
...
exports.uploadImage = (request, response) => {
  ...
  busboy.on("file", (fieldname, file, filename, encoding, mimetype) => {
    ...
    const imageExtension = filenameComponents[filenameComponents.length - 1];
  ...
  busboy.on("finish", () => {
    admin
      ...
      .then(() => {
        ...
        return db.doc(`/${constants.collection.users}/${request.user.handle}`).update({ imageUrl });
      })
      .then(() => {
...
exports.markNotificationsRead = (request, response) => {
  ...
  request.body.forEach((notificationId) => {
    const notification = db.doc(`/${constants.collection.notifications}/${notificationId}`);
